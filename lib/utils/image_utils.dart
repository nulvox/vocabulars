...
class ImageUtils {
  ...
  static Widget buildImage({
    ...
    if (kDebugMode) {
      AppLogger.debug('Building image from path: $assetPath');
    }
    ...
    try {
      ...
      if (kDebugMode) {
        AppLogger.debug('Error building image from path $assetPath: $e');
      }
      ...
      child: Center(
        child: Column(
          ...
          children: [
            ...
            ), // Using exact grey700 color value
            if (kDebugMode)
              Padding(
  ...
  static Widget loadAssetImage({
    ...
    if (kDebugMode) {
      AppLogger.debug('Loading asset image from path: $fullPath');
      AppLogger.debug('Image is vector: ${isVectorImage(fullPath)}');
      AppLogger.debug('Image is raster: ${isRasterImage(fullPath)}');
      AppLogger.debug('Image is supported: ${isSupportedImage(fullPath)}');
    }
